...
class Autocorrelogram(imfeat.BaseFeature):
    ...
    def make_features(self, image_cv):
        ...
        cdef np.ndarray distance_set = self.distance_set
        ...
        autocorrelogram(<np.uint8_t *>image_packed.data, image_cv.height, image_cv.width, self.unique_colors,
                        <int *>distance_set.data, distance_set.size, <np.float64_t *>out.data, out_ac_size)
